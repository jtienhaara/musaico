    ...
    implements SparseRegion, Serializable
{
        ...
        throws I18nIllegalArgumentException
    {
        ...
        // Sort the sub-regions and merge any that overlap or touch
        // borders.
        ...
        for ( ArrayRegion sub_region : flattened_sub_regions )
        {
            if ( last_sub_region != null )
            {
                ...
                if ( start_index <= ( last_sub_region.end ().index () + 1L ) )
                {
                    // Touching or overlapping regions.  Don't bother
        ...
        if ( this.subRegions.length > 0 )
        {
            ...
            this.size = this.space ().expr ( this.end ).subtract ( this.start );
        }
        else
        {
            // Empty sparse region.
                             ...
                             )
    {
        ...
        // Binary search.
        Search search =
            this.space ().search ( new SpecificPosition ( position ) );
        long sub_region_index =
            search.findSubRegionIndex ( this );
    ...
    {
        Search search =
            this.space ().search ( new SpecificPosition ( region.start () ) );
        long sub_region_index = search.findSubRegionIndex ( this );
        if ( sub_region_index < 0L
