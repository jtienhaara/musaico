    ...
    implements Region, Serializable
{
    ...
    private ArrayRegion ()
    {
        this.start = ArraySpace.STANDARD.origin ();
        this.end = this.start;
        this.size =
            new ArraySize ( 0L );
    }
    ...
    public int hashCode ()
    {
        long hash_code = this.start ().index () * 4099L;
        long end_index = this.end ().index ();
        hash_code += end_index;
        hash_code += end_index - this.size ().length ();
        hash_code = hash_code % (long) Integer.MAX_VALUE;
    ...
    /**
     * @see musaico.region.Region#search(musaico.region.Criterion)
     */
    @Override
