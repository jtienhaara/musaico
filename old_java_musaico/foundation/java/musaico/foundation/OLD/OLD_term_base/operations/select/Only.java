...
/**
 * <p>
 * Outputs the one and only element from the input, if the input
 * has exactly one element, or No elements at all, if the input
 ...
 */
public class Only<VALUE extends Object>
    ...
    implements ElementalOperation<VALUE, VALUE>, Serializable
{
               ...
               ReturnNeverNull.Violation
    {
        ...
        {
            // We can't output exactly one value if we can't even
            // count the number of elements.
            return input.head ( 0L ); // No only Term.
        }
        ...
        if ( num_elements == 0L
             || countable.length () != 1L )
        {
            return countable.head ( 0L ); // No only Term.
        }
        else
        {
            return input;
