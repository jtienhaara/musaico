    ...
    implements Conductor, Serializable
{
    ...
    {
        if ( wait_milliseconds < 0L )
        {
            this.waitMilliseconds = 0L;
        }
        else
        {
    ...
    {
        return (int) this.waitMilliseconds;
    }
    ...
    {
        return "wait [ " + this.id () + " ] [ " + this.waitMilliseconds + " ms ]";
    }

    private final void waitNow ()
    {
        final Object sleep_lock = new Object ();
        try
        {
            synchronized ( sleep_lock )
            {
                sleep_lock.wait ( this.waitMilliseconds );
            }
        }
