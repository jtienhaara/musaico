...
import musaico.foundation.term.Reducible;
import musaico.foundation.term.Term;


/**
 * <p>
 * Stores an Operation and an input to that Operation.
 * </p>
 *
 * <p>
...
public class Invocation<INPUT extends Object, OUTPUT extends Object>
    implements Reducible<OUTPUT>, Serializable
{
    ...
    /**
     * <p>
     * A child Reducibility on which an Invocation depends:
     * First the Invocation itself, then the input (which might be
     * Blocking, for example), then the Operation to be invoked
     * (which might be a BlockingOperation, for example), then the output
     * (which also might be Blocking, for example).  Each stage receives
     * its own Reducibility independently of the other 2.  A caller
     * might then choose to reduce an Invocation completely and inspect
     * the output final value.  Or the caller might choose to reduce
     * it to its constituents and leave it at that.  Or the caller
     * might reduce it and the input, but not the execution
     * of the operation, nor the output from that execution, leading
     * to a "lazy" result which can be reduced later by another party.
     * </p>
     */
    protected static abstract class AbstractStage<STAGE extends Object>
        implements Reducible<STAGE>, Serializable
    {
        private static final long serialVersionUID =
            Invocation.serialVersionUID;

        // Enforces constructor parameter obligations and so on for us.
        private static final Advocate classContracts =
            new Advocate ( Invocation.AbstractStage.class );


        // The parent Invocation to which this stage belongs.
        private final Invocation invocation;

        /**
         * <p>
         * Creates a new Invocation.AbstractStage.
         * </p>
         *
         * @param invocation The parent Invocation to which
         *                   this stage belongs.  Must not be null.
         */
                               ...
                               )
        {
            ...
            final Invocation.AbstractStage<?> that =
                (Invocation.AbstractStage<?>) object;

            if ( this.invocation == null )
            {
                if ( that.invocation != null )
                {
                    return false;
                }
            }
            else if ( that.invocation == null )
            {
                return false;
            }
            else if ( ! this.invocation.equals ( that.invocation ) )
            {
                return false;
            }
        ...
        public int hashCode ()
        {
            return
                this.invocation == null
                    ? 0
                    : this.invocation.hashCode ();
        }


        // Every Invocation.AbstractStage must implement reduce ().
        ...
        {
            final Multiplicity<VALUE> value =
                this.reduce ()
                    .value ();
            return value;
        }
    ...
    protected static class InputStage<STAGE extends Object>
     ...
     * @param input The input to pass to the Operation.  CAN be null.
     *
     * @param output The optional output that was returned by the
