 ...
 * contain a specific subset of indices.
 * </p>
 *
 *
 * <p>
    ...
    implements ArrayDomain, Serializable
{
    ...
    // The indices which form a subset of the indexable elements
    ...
    /**
     * <p>
     * Creates a new domain of arrays/collections containing at least
        ...
        throws NullPointerException
    {
        if ( indices == null )
        {
            throw new NullPointerException ( "Cannot create a ContainsIndices ( " + indices + " )" );
        }
                           ...
                           )
    {
        ...
        final ContainsIndices that = (ContainsIndices) obj;
            ...
            )
    {
        ...
        final List<Long> missing_indices =
            new ArrayList<Long> ();
        for ( int e = 0; e < this.indices.length; e ++ )
        {
            if ( ! found_flags.get ( e ) )
            {
                missing_indices.add ( this.indices [ e ] );
            }
        }

        if ( missing_indices.size () > 0 )
        {
            // Only add an array of the missing indices to the result.
            non_members.add (
