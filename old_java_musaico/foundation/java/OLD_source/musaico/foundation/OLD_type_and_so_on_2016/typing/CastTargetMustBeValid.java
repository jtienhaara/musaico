...
/**
 *  advance what class a Cast expects as target, so we might
 ...
 */
public class CastTargetMustBeValid
    extends AbstractTypingContract<CastValue>
{
    ...
    public static final CastTargetMustBeValid CONTRACT =
        new CastTargetMustBeValid ();
    ...
    public FilterState filter (
                               CastValue cast_target
                               )
    {
        if ( cast_target == null )
        {
            // Filter out null cast targets.
            return FilterState.DISCARDED;
        }

        final Type<?> expected_type =
            cast_target.cast ().type ().outputType ();
        final Value<?> actual_value = cast_target.value ();
